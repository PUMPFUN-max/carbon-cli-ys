...
pub fn parse(path: String, output: String, as_crate: bool) -> Result<()> {
    ...
    if as_crate {
        let lib_rs_content = format!(
            "pub struct {decoder_name};\npub mod accounts;\npub mod instructions;\npub mod types;\n\nuse solana_pubkey::pubkey;\n\nuse solana_pubkey::Pubkey;\n\npub const PROGRAM_ID: Pubkey = pubkey!(\"11111111111111111111111111111111\");\n\n// Re-export the yellowstone-specific functionality\npub use instructions::{{CompiledInstructionDecoder, CompiledInstructionParamsDecoder, CompiledInstructionWithAccountsDecoder, CompiledDecodedInstruction, ResolvedDecodedInstruction, CompiledInstructionAccounts, ResolvedInstructionAccounts}};\n",
...
pub fn scaffold(
    ...
    // Generate Cargo.toml
    let (carbon_deps_version, sol_deps_version) = ("0.8.1", "=2.1.15");
...
"#,
        ...
        grpc_deps = if data_source == "yellowstone-grpc" {
            r#"yellowstone-grpc-client = { version = "6.1.0" }
